...
func TestReassignPartitions_Success(t *testing.T) {
	...
	r := reassignPartitions{Cmd: base.Cmd{TopicCli: mockTopicCli}, topics: topicRegex, brokerIds: brokerIds, batch: batch, timeoutPerBatchInS: timeoutPerBatch, pollIntervalInS: pollInterval, throttle: throttle, zookeeper: zookeeper}
...
func TestReassignPartitions_ListFailure(t *testing.T) {
	...
	defer patch.Unpatch()
	r := reassignPartitions{Cmd: base.Cmd{TopicCli: mockTopicCli}, topics: "topic-1", brokerIds: brokerIds, batch: batch, timeoutPerBatchInS: timeoutPerBatch, pollIntervalInS: pollInterval, throttle: throttle, zookeeper: zookeeper}
